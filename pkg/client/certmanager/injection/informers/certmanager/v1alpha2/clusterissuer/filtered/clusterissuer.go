...
func Get(ctx context.Context, selector string) v1alpha2.ClusterIssuerInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch knative.dev/net-certmanager/pkg/client/certmanager/informers/externalversions/certmanager/v1alpha2.ClusterIssuerInformer with selector %s from context.", selector)
	}
